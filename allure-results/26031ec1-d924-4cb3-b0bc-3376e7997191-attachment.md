...
- Name: product/product.spec.js >> TC_PDT_03 Verify quantity update functionality
- Location: tests/product/product.spec.js:40:6
...
Call log:
  - waiting for locator('#addtocart_31_EnteredQuantity')
...
```ts
  ...
  25  | 
  26  | test('TC_PDT_02 Verify product price is correct', async ({ page }) => {
  27  | 
  ...
  39  | 
  40  | test.only('TC_PDT_03 Verify quantity update functionality', async ({ page }) => {
  41  | 
