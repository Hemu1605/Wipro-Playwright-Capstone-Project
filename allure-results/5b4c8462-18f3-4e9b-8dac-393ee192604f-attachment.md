...
- Name: profile/profile.spec.js >> TC_PRF_01 Verify User Login Before Profile Access
- Location: tests/profile/profile.spec.js:12:6
...
Expected: visible
Error: strict mode violation: locator('.account') resolved to 2 elements:
    1) <a class="account" href="/customer/info">hemrajgujjar0808@gmail.com</a> aka getByRole('link', { name: 'hemrajgujjar0808@gmail.com' })
    2) <a class="account" href="/customer/info">My account</a> aka getByRole('link', { name: 'My account' })
...
- generic [ref=e2]:
  - generic [ref=e3]:
    - generic [ref=e4]:
      ...
      - list [ref=e10]:
        - listitem [ref=e11]:
          - link "hemrajgujjar0808@gmail.com" [ref=e12] [cursor=pointer]:
            - /url: /customer/info
        - listitem [ref=e13]:
...
```ts
  ...
  2  | 
  3  | const { ProfilePage } = require('../../pages/ProfilePage');
  4  | 
  5  | test.describe('User Profile Test Cases', () => {
  6  | 
  7  | });
  8  | 
  9  | 
  10 | // TC_PRF_01 → Verify User Login Before Profile Access
  11 | 
  12 | test.only('TC_PRF_01 Verify User Login Before Profile Access', async ({ page }) => {
  13 | 
  14 |     const profilePage = new ProfilePage(page);
  15 | 
  16 |     // Open login page
  17 |     await profilePage.gotoLoginPage();
  18 | 
  19 |     // Login user
  20 |     await profilePage.login(
  21 |         process.env.USER_EMAIL,
  22 |         process.env.USER_PASSWORD
  23 |     );
  24 | 
  25 |     // Verify account visible
  26 |     await expect(profilePage.accountLink)
> 27 |         .toBeVisible();
     |          ^ Error: expect(locator).toBeVisible() failed
  28 | });
```